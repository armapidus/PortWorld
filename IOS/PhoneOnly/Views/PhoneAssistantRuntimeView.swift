...
struct PhoneAssistantRuntimeView: View {
  ...
  init(viewModel: PhoneAssistantRuntimeViewModel) {
      ...
      ScrollView(showsIndicators: false) {
        VStack(alignment: .leading, spacing: 18) {
          VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Minimal standalone reference app for the current phone-only assistant runtime.")
    ...
    .safeAreaInset(edge: .bottom) {
      VStack(spacing: 10) {
        if store.canActivate {
          ...
          .buttonStyle(.plain)
          .background(Color.phoneOnlyPrimary)
