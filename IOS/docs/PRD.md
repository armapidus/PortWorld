...
| `FR-16` | Query bundle upload uses 2 retry attempts with backoff on HTTP 429 / 5xx. On final failure, the user sees a non-intrusive dismissable error. |
| `FR-17` | All temp files (WAV, MP4) produced by a query cycle are deleted after a successful upload and swept on app launch. |
| `FR-18` | Per `PRD_APPENDIX_INTERFACES.md` wire contract: `POST /query` parts are `metadata` (JSON), `audio` (WAV), `video` (MP4) in that order. |
...
| `v2.0` | Automatic voice wake word without button; multi-sensor fusion; Android glasses support (if Meta extends DAT SDK) |
