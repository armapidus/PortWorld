        ...
        while True:
            ...
            if raw_message is None:
                ...
                try:
                    ...
                    logger.warning(f"Non-UTF8 WebSocket payload from {active_session_id}")
                    continue
...
async def handle_client_message(
    ...
    if msg_type == "query.bundle.uploaded":
        ...
        logger.info(f"Query bundle uploaded notification from {session_state.session_id}: {payload}")
        return
...
async def stream_audio_to_session(
    ...
    try:
        ...
        logger.info(f"Streamed {len(audio_chunks)} audio chunks to session {session_id}")
        return True
